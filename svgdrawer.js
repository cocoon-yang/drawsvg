...
SVGDrawer.prototype.open = function( fileName )
{
	var self = this;
	if( undefined == fileName )
	{
		console.log("SVGDrawer::open: file name is undefined, please set valid file name.");
		return;
	}
	...
	try {
 	 ...
 	 /* Handle the error */
		console.log("SVGDrawer::open: open file with the name", self._fileName, " failed.");
		return;
	}   
}
...
SVGDrawer.prototype.setFileName = function( fileName )
{
	...
	if( undefined == fileName )
	{
		console.log("SVGDrawer::setFileName: file name is undefined, please set a valid file name.");
		return;
	}
	self._fileName = fileName;
...
SVGDrawer.prototype.iterateJSON = function( theJSON )
{
	var self = this;
	var str = "";
     ...
     { 
         ...
         {
               str += self.iterateJSON( theJSON[id] )
          }else{
			 //console.log( id + "=\"" + theJSON[id] + "\""); 
			 str += ( id + "=\"" + theJSON[id] + "\" " );
          }
	 }
	 return str;
...
SVGDrawer.prototype.element = function( name, attribute, indent  )
{
	...
	str += " " + self.iterateJSON( attribute );
    str += '>\n'
...
//
//  http://corpus.hubwiz.com/2/node.js/14974864.html
//
//  var object3 = extend({}, object1, object2);
//
SVGDrawer.prototype.extend = function(target) {
...
// 
//  Inputs:
//     x1, y1: the coordinates of the start point of the line 
//     x2, y2: the coordinates of the end point of the line
//     style:  the style of the line
//
SVGDrawer.prototype.line = function( x1, y1, x2, y2, style )
{
	if( x1 == undefined )
	{
		console.log("SVGDrawer::line: invalid parameter.");
	...
	if( style == undefined )
	{
        ...
        theAttr={};
        theAttr['x1'] = parseFloat(x1);
        theAttr['y1'] = parseFloat(y1);
        theAttr['x2'] = parseFloat(x2);
		theAttr['y2'] = parseFloat(y2);
		var newAttr = this.extend( {}, theAttr, style);		
        this.element('line', newAttr );
        this.popData(); 
}
...
// SVGDrawer.prototype.element_old = function( name, attribute, indent  )
// {
//      var self = this;
...
SVGDrawer.prototype.rot = function( x, y, theta, x0, y0 )
{
  if( x == undefined )
  {
    return;
  ...
  var s = Math.sin( parseFloat( theta ) );    
  var c = Math.cos( parseFloat( theta ) );

  var x1 = parseFloat( x ) * c + parseFloat( y ) * s + x0;
  var y1 = parseFloat( x ) *(-s) + parseFloat( y ) * c + y0;
...
SVGDrawer.prototype.coordinateaxis= function( parameterArray )
{
	...
	var x_range = parseInt(x2) - parseInt(x1);
	var y_range = parseInt(y2) - parseInt(y1);
	...
	for( var i = 1; i < step; i++ )
	{
		...
		var x = parseInt(x1) + x_scale * i ;
		var y = parseInt(y1) + y_scale * i ;
...
SVGDrawer.prototype.drawHorizontalGridArray = function( x, y, array )
{
	...
	for( var i = 0; i < length; i++ )
	{
		...
		console.log( (theX + 8 ), ( theY +23), 20, array[i] );

		var theAttr={};
		theAttr['x'] = (theX + 8 );
		theAttr['y'] = ( theY +23);
		theAttr['fill'] = 'black';
		theAttr['font-size'] = 20;
		theAttr['content'] = array[i];
		self.element('text', theAttr )
                self.popData(); 
